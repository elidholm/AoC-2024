...
fn print_hike(map: &Map, hike: &Hike) {
    let mut i: usize = 0;
    for line in map {
        let mut j: usize = 0;
        for c in line {
            if hike.visited.contains(&(i, j)) {
                print!("O");
            } else {
                print!("{}", c);
            }
...
fn part1(input: &str) -> i32 {
    ...
    let mut nuts: HashMap<(usize, usize), i32> = HashMap::new();
    deez.push(start);
    nuts.insert(start, 0);
    ...
    while hikes.len() > 0 {
        ...
        for (next, dir) in possible_steps {
            ...
            deez.push(next);
            if nuts.contains_key(&next) {
                if nuts.get(&next).unwrap() <= &new_cost {
                    continue;
                }
            } else {
                nuts.insert(next, new_cost);
            }
